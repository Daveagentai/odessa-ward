...
## Version 1.4 — Snapshot: May 3, 2026
...
### Tables & Row Counts (as of May 3, 2026)

| Table | Rows | RLS |
|---|---|---|
| members | 570 (569 active + 1 friend) | Yes |
| households | 328 | Yes |
| ministering_assignments | ~274 | No |
| calling_assignments | 167 | Yes |
| callings | 84 | Yes |
| member_contact_changes | 6 | Yes |
| tasks | 17 | Yes |
| profiles | 12 | Yes |
| visits | 13 | Yes |
...
### New Columns (added May 3, 2026)

**calling_assignments**
- `archived_at TIMESTAMPTZ` — when the assignment was archived (hidden from active queue)
- `archived_by UUID REFERENCES profiles(id)` — who archived it
- Partial index `idx_calling_assignments_active ON (status, member_id) WHERE archived_at IS NULL` for fast active-queue queries
...
### May 3, 2026 (this session)
- Feature: Calling archive workflow — Withdrawn/Declined callings can be archived (hidden from queue) and unarchived
  - DB migration: `archived_at`, `archived_by` columns + partial index `idx_calling_assignments_active`
  - Calling queue: hides archived rows by default with "Show Archived" toggle
  - Calling detail: Archive button (roles 1-3 only), Unarchive button, archived banner shown when applicable
  - General Notes hidden unless status in {Active, Sustained, Set Apart}
- Feature: Member detail Workflow Callings card — active workflow rows + collapsible "Past Callings" (Released only)
- Bug fix: replaced `pe(c.released_date)` in YO with `new Date(c.released_date).toLocaleDateString()` (`pe` not aliased in YO scope)
- LCR import: 566 LCR-active members updated with `lcr_last_seen_at = 2026-05-03 ~15:25 UTC`
- LCR import script fix (`lcr_import_v2.py`): now matches on `COALESCE(original_full_name, full_name) = l.full_name` to handle the trigger-overwritten `full_name` field. Previously matched on `full_name` directly, which caused 301 false-duplicate inserts because the trigger overwrites `full_name` with `preferred_name` while storing the long-form LCR name in `original_full_name`.
- Backup: full DB snapshot (8 data tables + 7 schema files) committed to `/backup/2026-05-03/`
